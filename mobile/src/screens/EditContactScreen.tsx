...
export default function EditContactScreen({ route, navigation }: any) {
  ...
  const appId =
    contact.application && typeof contact.application === 'object'
      ? (contact.application as ApplicationRef)._id
      : (contact.application as string) ?? '';
          ...
          <View style={styles.card}>
            <View style={styles.fieldWrap}>
              ...
              <PickerModal
                ...
                onChange={(v) => { setDirty(true); setForm((f) => ({ ...f, application: v })); }}
                clearable
              />
