...
export default function EditInterviewScreen({ route, navigation }: any) {
  const interview: Interview = route.params.interview;
  ...
  const appId =
    interview.application && typeof interview.application === 'object'
      ? (interview.application as ApplicationRef)._id
      : (interview.application as string) ?? '';

  const [form, setForm] = useState({
    application: appId,
    date: dayjs(interview.scheduledAt).format('YYYY-MM-DD'),
    time: dayjs(interview.scheduledAt).format('HH:mm'),
    type: interview.type,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setServerError(err.response?.data?.error || 'Failed to save changes. Please try again.');
    } finally {
      setLoading(false);
    }
  };
